...
export function CustomerJobEntityTable<
  T extends { id: string | number; status?: string }
>({
  columns,
  data = [],
  ...
  const handleRowToggle = (rowId: string | number) => {
    console.log(
      "handleRowToggle called with rowId:",
      rowId,
    ...
    setExpandedRowId((prev) => {
      ...
      if (isExpanding && contentRefs.current[rowId]) {
        ...
        console.log("Setting content height for row", rowId, height);
      }
    ...
    >
      {selectedRows.length > 0 && (
        <div className="px-6 py-3 bg-primary/10 border-b border-primary/20 flex justify-between items-center">
          ...
          {/* <button
            className="px-4 py-2 text-sm font-semibold rounded-lg text-white bg-primary/80 hover:bg-primary focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background transition-colors disabled:opacity-50"
                            ...
                            return (
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (isViewAction) {
                                    console.log("View clicked for row", row.id);
                                    handleRowToggle(row.id);
                                  } else {
                                    action.onClick(row);
                                  }
